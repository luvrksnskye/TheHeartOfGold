...
class NewsCarousel {
    constructor() {
        ...
        this.newsData = [
            { id: '01', date: 'February, 2025', title: 'Demo Launch Announcement Event', description: 'Experience the first playable demo of The Heart of Gold. Face waves of enemies and discover the combat system.' },
            { id: '02', date: 'January 21, 2025', title: 'Meet the Characters', description: 'Discover the unique heroes of The Heart of Gold, each with their own combat style and abilities.' },
            { id: '03', date: 'January, 2025', title: 'Development Update', description: 'Behind the scenes look at the development progress and upcoming features.' }
        ];
    }
    ...
    createPanel(n, i) {
        return `
            <article class="news-panel ${i === 0 ? 'active' : ''}" data-index="${i}" data-blog-id="${n.id}">
                <div class="panel-bg">
    ...
    createIndicators(parent) {
        ...
        wrap.innerHTML = this.newsData.map((_, i) => 
            `<button class="news-indicator clickable ${i === 0 ? 'active' : ''}" data-index="${i}" aria-label="Go to news ${i + 1}"></button>`
        ).join('');
    ...
    updatePanelWidth() {
        if (this.panels.length > 0) {
            this.panelWidth = this.panels[0].offsetWidth + this.gap;
        }
    }
    ...
    navigate(dir) {
        ...
        // Loop: Si va más allá del final, vuelve al inicio
        if (newIndex >= this.newsData.length) {
            newIndex = 0;
        } else if (newIndex < 0) {
            // Si va antes del inicio, va al final
            newIndex = this.newsData.length - 1;
        }
